  ...
  methods: {
    ...
    async fetchDownloadLink() {
      ...
      if (this.downloadAsZip && response.data.download_link) {
        this.downloadLink = `${response.data.download_link}.zip`; // Append .zip extension
      } else {
        this.downloadLink = response.data.download_link; // Set the download link
      }
